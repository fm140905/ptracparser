...
{
  if (inputFile.fail()) {
    std::cerr << "INP file not found." << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
void MCNPGeometry::associateCell2Density()
{
  ...
  if (iss) {
    ...
    if (matNum == 0) {
      ...
      istest >> fdensity;
      if (istest.fail()) {
        std::cerr << "WRONG DENSITY / CELL Definition" << std::endl;
...
void MCNPGeometry::addCell2Density(unsigned long key, const std::pair<unsigned long, std::string> &value)
{
  if (cell2Density.find(key) == cell2Density.end()) {
    ...
    std::cerr << "This cellID " << key << " already appeared in the MCNP input file." << std::endl;
    std::cerr << "Check MCNP input file for errors..." << std::endl;
  }
}
